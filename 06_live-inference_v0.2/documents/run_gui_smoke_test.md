...
- Make `06_live-inference_v0.2/src` importable with `PYTHONPATH`.
...
```bash
06_live-inference_v0.2/models/selections/current.toml
```

That selection should point at staged artifact directories under:

```bash
06_live-inference_v0.2/models/
```
...
```bash
06_live-inference_v0.2/config/synthetic_camera.toml.example
```

It reads a small demo source directory:

```bash
06_live-inference_v0.2/demo/synthetic_camera_source/
```
...
```bash
PYTHONPATH=06_live-inference_v0.2/src ./.venv/bin/python -m live_inference.gui.app
```

Optional auto-start smoke command:

```bash
PYTHONPATH=06_live-inference_v0.2/src ./.venv/bin/python -m live_inference.gui.app --auto-start-camera --auto-start-inference
```

Mini PC / CPU auto-start smoke command:

```bash
PYTHONPATH=06_live-inference_v0.2/src ./.venv/bin/python -m live_inference.gui.app --device cpu --auto-start-camera --auto-start-inference
```

Useful manual-test options:

```bash
--synthetic-camera-config 06_live-inference_v0.2/config/synthetic_camera.toml.example
--model-selection 06_live-inference_v0.2/models/selections/current.toml
...
```bash
PYTHONPATH=06_live-inference_v0.2/src ./.venv/bin/python \
...
## ROI Locator Diagnostics

Use this workflow before running the full distance/orientation path when ROI-FCN
is localising the wrong region:

1. Click `Stop Inference`.
2. Click `Capture Frame`.
3. Open `Diagnostics`.
4. Select `Sheet dark foreground`.
5. Click `Preview Locator Input`.
6. Adjust `Sheet min gray` and `Target max gray` until the Defender-like target is bright and the background is dark.
7. Click `Run ROI Locator Only`.
8. If the predicted centre and requested ROI are sane, click `Run Single Inference`.
9. Enable `Record Trace` when saving a diagnostic bundle for review.

## Expected Behaviour

- The synthetic camera repeatedly publishes the copied demo images into
  `06_live-inference_v0.2/live_frames/`.
...
- ROI locator diagnostics are intentionally simple and do not perform adaptive
  background modelling or model retraining.
