...
import live_inference.engines.torch_tri_stream_engine as engine_module  # noqa: E402
...
class TorchTriStreamInferenceEngineUnitTests(unittest.TestCase):
    ...
    def test_default_selection_is_scoped_to_current_live_project(self) -> None:
        self.assertEqual(
            engine_module.DEFAULT_SELECTION_PATH,
            Path("models/selections/current.toml"),
        )

        selected_root, _device = engine_module._resolve_selected_model_root_and_device(
            None
        )

        self.assertEqual(selected_root, KNOWN_DISTANCE_ORIENTATION_ROOT.resolve())
