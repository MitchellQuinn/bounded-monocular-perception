...
class SingleFrameInferenceRunnerTests(unittest.TestCase):
    ...
    def test_preview_locator_input_uses_diagnostic_path_without_engine(self) -> None:
        preprocessor = FakePreprocessor()
        engine = FakeEngine()
        runner = _runner(preprocessor, engine)

        outcome = runner.preview_locator_input(b"frame")

        self.assertIsNotNone(outcome.result)
        self.assertIsNone(outcome.error)
        self.assertEqual(len(preprocessor.calls), 0)
        self.assertEqual(len(preprocessor.preview_locator_calls), 1)
        self.assertEqual(len(engine.calls), 0)

    def test_run_roi_locator_only_records_trace_metadata_without_engine(self) -> None:
        with TemporaryDirectory() as tmp_dir:
            preprocessor = FakePreprocessor()
            engine = FakeEngine()
            runner = _runner(
            ...
            outcome = runner.run_roi_locator_only(b"frame", record_trace=True)

            self.assertIsNotNone(outcome.result)
            self.assertIsNone(outcome.error)
            self.assertEqual(len(preprocessor.calls), 0)
            self.assertEqual(len(preprocessor.locator_only_calls), 1)
            self.assertEqual(len(engine.calls), 0)
            assert outcome.trace_path is not None
            self.assertTrue((outcome.trace_path / "final_locator_input.png").is_file())
            self.assertTrue((outcome.trace_path / "roi_fcn_metadata.json").is_file())
...
class FakePreprocessor:
    def __init__(self, *, exception: Exception | None = None) -> None:
        ...
        self.preview_locator_calls: list[tuple[InferenceRequest, bytes]] = []
        self.locator_only_calls: list[tuple[InferenceRequest, bytes]] = []
    ...
    def preview_roi_locator_input(
        ...
        self.preview_locator_calls.append((request, image_bytes))
        return _locator_diagnostic(request)
    ...
    def run_roi_locator_only(
        ...
        self.locator_only_calls.append((request, image_bytes))
        return _locator_diagnostic(request)
...
class _LocatorDiagnosticPayload:
    def __init__(
        ...
        *,
        ...
        self.input_image_hash = request.frame.frame_hash
        self.locator_input_image = [[0]]
        self.debug_paths = debug_paths
        self.preprocessing_metadata = {
            "preprocessing_contract_name": "test-preprocess",
            "roi_locator_input_mode": "sheet_dark_foreground",
            "roi_locator_input_polarity": "sheet_dark_foreground",
            "roi_confidence": 0.75,
            "roi_center_source_xy_px": (10.0, 12.0),
            "roi_locator_center_source_xy_px": (10.0, 12.0),
            "roi_request_xyxy_px": (0.0, 0.0, 20.0, 24.0),
            "roi_accepted": True,
...
def _locator_diagnostic(request: InferenceRequest) -> _LocatorDiagnosticPayload:
    debug_paths: dict[str, Path] = {}
    if request.save_debug_images and request.debug_output_dir is not None:
        final = Path(request.debug_output_dir) / "source-final-locator.png"
        final.write_bytes(b"locator")
        debug_paths["final_locator_input"] = final
    return _LocatorDiagnosticPayload(request=request, debug_paths=debug_paths)
