...
class LiveInferenceMainWindowTests(unittest.TestCase):
    ...
    def test_roi_locator_controls_exist(self) -> None:
        ...
        self.assertIsNotNone(
            self.window.findChild(QSpinBox, "target_max_gray_input")
        )
        self.assertIsNotNone(
            self.window.findChild(QSpinBox, "morphology_close_kernel_px_input")
        )
        self.assertIsNotNone(
            self.window.findChild(QSpinBox, "dilate_kernel_px_input")
        )
        self.assertIsNotNone(
            self.window.findChild(QPushButton, "preview_locator_input_button")
        )
        self.assertIsNotNone(
            self.window.findChild(QPushButton, "run_roi_locator_only_button")
        )
        self.assertIsNotNone(
            self.window.findChild(QLabel, "roi_locator_next_action_label")
        )
        self.assertIsNotNone(
    ...
    def test_roi_locator_controls_update_shared_state(self) -> None:
        ...
        mode.setCurrentText("Sheet dark foreground")
        _process_events(self.app)
    ...
    def test_roi_locator_threshold_controls_update_shared_state(self) -> None:
        ...
        sheet = self.window.findChild(QSpinBox, "sheet_min_gray_input")
        target = self.window.findChild(QSpinBox, "target_max_gray_input")
        close = self.window.findChild(QSpinBox, "morphology_close_kernel_px_input")
        dilate = self.window.findChild(QSpinBox, "dilate_kernel_px_input")
        ...
        snapshot = self.window.stage_policy_state.get_snapshot()
        self.assertEqual(snapshot.sheet_min_gray, 201)
        self.assertEqual(snapshot.target_max_gray, 111)
        self.assertEqual(snapshot.morphology_close_kernel_px, 5)
        self.assertEqual(snapshot.dilate_kernel_px, 3)
    ...
    def test_preview_locator_input_uses_captured_frame_and_updates_status(self) -> None:
        image_path = self._write_temp_frame_image()
        frame_bytes = image_path.read_bytes()
        runner = _FakeSingleFrameRunner()
        self.window.frame_reader = _FakeFrameReader(
        ...
        self.window.preview_locator_input_button.click()
        _process_events(self.app)

        self.assertEqual(runner.preview_locator_calls, [frame_bytes])
        self.assertIn("Mode sheet_dark_foreground", self.window.roi_locator_next_action_label.text())
        self.assertEqual(self.window.roi_confidence_value.text(), "0.750")

    def test_run_roi_locator_only_uses_captured_frame_without_single_inference(self) -> None:
        image_path = self._write_temp_frame_image()
        frame_bytes = image_path.read_bytes()
        runner = _FakeSingleFrameRunner()
        self.window.frame_reader = _FakeFrameReader(
        ...
        self.assertEqual(runner.roi_locator_only_calls, [frame_bytes])
    ...
    def test_locator_diagnostics_require_captured_frame(self) -> None:
        ...
        self.assertEqual(runner.roi_locator_only_calls, [])
        self.assertIn(
...
class _FakeSingleFrameRunner:
    def __init__(self, *, trace_path: Path | None = None) -> None:
        ...
        self.preview_locator_calls: list[bytes] = []
        self.roi_locator_only_calls: list[bytes] = []
    ...
    def preview_locator_input(
        ...
        *,
        ...
        self.preview_locator_calls.append(bytes(image_bytes))
        self.record_trace_values.append(bool(record_trace))
        return _SingleFrameOutcomePayload(
            result=_fake_locator_diagnostic(),
            error=None,
            trace_path=self.trace_path,
        )
    ...
    def run_roi_locator_only(
        ...
        *,
        ...
        self.roi_locator_only_calls.append(bytes(image_bytes))
        self.record_trace_values.append(bool(record_trace))
        return _SingleFrameOutcomePayload(
            result=_fake_locator_diagnostic(),
            error=None,
...
def _fake_locator_diagnostic() -> _LocatorDiagnosticPayload:
    return _LocatorDiagnosticPayload(
        locator_input_image=np.zeros((48, 80), dtype=np.uint8),
        preprocessing_metadata={
            ...
            contracts.PREPROCESSING_METADATA_SOURCE_IMAGE_WIDTH_PX: 80,
            contracts.PREPROCESSING_METADATA_SOURCE_IMAGE_HEIGHT_PX: 48,
            contracts.PREPROCESSING_METADATA_ROI_CONFIDENCE: 0.75,
