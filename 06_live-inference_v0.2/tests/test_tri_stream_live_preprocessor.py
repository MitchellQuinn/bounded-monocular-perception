...
class TriStreamLivePreprocessorTests(unittest.TestCase):
    ...
    def test_preview_roi_locator_input_writes_final_locator_artifact(self) -> None:
        image_bytes = _fixture_image_bytes()
        with TemporaryDirectory() as tmp_dir:
            request = _request(
                image_bytes,
            ...
            result = TriStreamLivePreprocessor(
                model_manifest=_fixture_manifest(ORIENTATION_SOURCE_RAW_GRAYSCALE),
                roi_locator=FakeDebugRoiLocator(),
                stage_policy=StageTransformPolicySnapshot(
                    roi_locator_input_mode=ROI_LOCATOR_INPUT_MODE_SHEET_DARK_FOREGROUND
                ),
            ).preview_roi_locator_input(request, image_bytes)

            self.assertIsInstance(result, RoiLocatorDiagnosticResult)
            self.assertIn(ARTIFACT_FINAL_LOCATOR_INPUT, result.debug_paths)
            self.assertTrue(Path(result.debug_paths[ARTIFACT_FINAL_LOCATOR_INPUT]).is_file())
            self.assertIn(
                ARTIFACT_LOCATOR_INPUT_SHEET_DARK_FOREGROUND,
                result.debug_paths,
            )
            self.assertEqual(
                result.preprocessing_metadata["roi_locator_input_mode"],
                ROI_LOCATOR_INPUT_MODE_SHEET_DARK_FOREGROUND,
            )

    def test_run_roi_locator_only_records_confidence_center_and_bounds(self) -> None:
        image_bytes = _fixture_image_bytes()
        with TemporaryDirectory() as tmp_dir:
            request = _request(
                image_bytes,
            ...
            result = TriStreamLivePreprocessor(
                model_manifest=_fixture_manifest(ORIENTATION_SOURCE_RAW_GRAYSCALE),
                roi_locator=LowConfidenceRoiLocator(),
            ).run_roi_locator_only(request, image_bytes)

            metadata = result.preprocessing_metadata
            self.assertIsNotNone(result.roi_location)
            self.assertEqual(metadata[contracts.PREPROCESSING_METADATA_ROI_CONFIDENCE], 0.12)
            self.assertEqual(metadata["roi_center_source_xy_px"], (240.0, 150.0))
            self.assertIn(contracts.PREPROCESSING_METADATA_ROI_REQUEST_XYXY_PX, metadata)
            self.assertFalse(metadata[contracts.PREPROCESSING_METADATA_ROI_ACCEPTED])
            self.assertFalse(metadata["distance_orientation_regressor_reached"])
            self.assertIn(ARTIFACT_FINAL_LOCATOR_INPUT, result.debug_paths)

    def test_preprocessing_failure_after_roi_preserves_roi_metadata_and_debug_paths(self) -> None:
        image_bytes = _fixture_image_bytes()
        with TemporaryDirectory() as tmp_dir:
            request = _request(
                image_bytes,
            ...
            with self.assertRaises(PreprocessingDebugError) as raised:
                FailingOrientationPreprocessor(
                    model_manifest=_fixture_manifest(ORIENTATION_SOURCE_RAW_GRAYSCALE),
                    roi_locator=FakeDebugRoiLocator(),
                    stage_policy=StageTransformPolicySnapshot(
                        roi_locator_input_mode=ROI_LOCATOR_INPUT_MODE_SHEET_DARK_FOREGROUND
            ...
            metadata = raised.exception.preprocessing_metadata
            self.assertEqual(
                metadata["roi_locator_input_mode"],
                ROI_LOCATOR_INPUT_MODE_SHEET_DARK_FOREGROUND,
            )
            self.assertEqual(metadata[contracts.PREPROCESSING_METADATA_ROI_CONFIDENCE], None)
            self.assertEqual(metadata["roi_center_source_xy_px"], (240.0, 150.0))
            self.assertFalse(metadata["distance_orientation_regressor_reached"])
...
class FailingOrientationPreprocessor(TriStreamLivePreprocessor):
    def _build_orientation_image(  # type: ignore[override]
        self,
        *,
