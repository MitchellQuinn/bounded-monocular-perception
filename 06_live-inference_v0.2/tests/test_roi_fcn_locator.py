...
class RoiFcnLocatorGeometryTests(unittest.TestCase):
    def test_as_is_locator_representation_returns_original_grayscale(self) -> None:
        source = np.asarray([[0, 32, 255], [10, 120, 200]], dtype=np.uint8)

        output = build_roi_locator_input_representation(source, mode="as_is")

        self.assertTrue(np.array_equal(output, source))
        self.assertIsNot(output, source)

    def test_inverted_locator_representation_returns_inverse_grayscale(self) -> None:
        source = np.asarray([[0, 32, 255], [10, 120, 200]], dtype=np.uint8)

        output = build_roi_locator_input_representation(source, mode="inverted")

        self.assertTrue(np.array_equal(output, 255 - source))

    def test_sheet_dark_foreground_promotes_dark_object_on_bright_sheet(self) -> None:
        source = np.zeros((120, 160), dtype=np.uint8)
        source[40:100, 30:130] = 220
        source[60:82, 70:95] = 40

        output = build_roi_locator_input_representation(
            source,
            mode="sheet_dark_foreground",
            sheet_min_gray=180,
            target_max_gray=100,
            min_component_area_px=20,
            morphology_close_kernel_px=3,
            dilate_kernel_px=0,
        )

        self.assertEqual(int(output[70, 80]), 255)
        self.assertEqual(int(output[10, 10]), 0)
        self.assertGreater(int(np.count_nonzero(output)), 0)

    def test_sheet_dark_foreground_suppresses_dark_clutter_outside_sheet(self) -> None:
        source = np.zeros((120, 160), dtype=np.uint8)
        source[8:25, 8:25] = 20
        source[45:105, 40:140] = 230
        source[70:88, 80:110] = 35
        ...
        self.assertEqual(int(output[15, 15]), 0)
        self.assertEqual(int(output[78, 90]), 255)

    def test_sheet_dark_foreground_thresholds_affect_output(self) -> None:
        source = np.zeros((80, 100), dtype=np.uint8)
        source[20:70, 20:90] = 210
        source[40:55, 45:65] = 100

        suppressed = build_roi_locator_input_representation(
            source,
            mode="sheet_dark_foreground",
            sheet_min_gray=180,
            target_max_gray=80,
        ...
        included = build_roi_locator_input_representation(
            source,
            mode="sheet_dark_foreground",
            sheet_min_gray=180,
            target_max_gray=120,
        ...
        self.assertEqual(int(suppressed[45, 50]), 0)
        self.assertEqual(int(included[45, 50]), 255)

    def test_sheet_dark_foreground_morphology_controls_do_not_crash(self) -> None:
        source = np.zeros((80, 100), dtype=np.uint8)
        source[20:70, 20:90] = 210
        source[40:55, 45:65] = 70

        output = build_roi_locator_input_representation(
            source,
            mode="sheet_dark_foreground",
            morphology_close_kernel_px=4,
            dilate_kernel_px=2,
        )
    ...
    def test_invalid_locator_representation_mode_fails_clearly(self) -> None:
        with self.assertRaisesRegex(ValueError, "roi_locator_input_mode"):
            build_roi_locator_input_representation(
