...
@dataclass(frozen=True)
class SingleFrameLocatorDiagnosticOutcome:
    """Result payload for captured-frame ROI locator diagnostics."""
...
class SingleFrameInferenceRunner:
    ...
    def preview_locator_input(
        ...
        *,
        ...
        """Build the final ROI-FCN locator input for the supplied frozen frame."""
        return self._run_locator_diagnostic(
            image_bytes,
            source_path=source_path,
            frame_metadata=frame_metadata,
            record_trace=record_trace,
            preprocessor_method_name="preview_roi_locator_input",
            request_extra={"locator_preview": True},
        )
    ...
    def run_roi_locator_only(
        ...
        *,
        ...
        """Run ROI-FCN localization without tri-stream preprocessing or inference."""
        return self._run_locator_diagnostic(
            image_bytes,
            source_path=source_path,
            frame_metadata=frame_metadata,
            record_trace=record_trace,
            preprocessor_method_name="run_roi_locator_only",
            request_extra={"locator_only": True},
        )

    def _run_locator_diagnostic(
        self,
        image_bytes: bytes,
        *,
            ...
            if self._trace_recorder is None:
                ...
                try:
                    ...
                    trace_setup_error = self._worker_error(
                        ...
                        failure_stage=FrameFailureStage.OUTPUT,
                        error_type="trace_recording_failed",
                        message=f"Could not create locator diagnostic trace directory: {exc}",
                        exception=exc,
                    )
        ...
        method = getattr(self._preprocessor, preprocessor_method_name, None)
        if not callable(method):
            error = self._worker_error(
                ...
                error_type="locator_diagnostic_unavailable",
                message=(
                    "ROI locator diagnostic is unavailable: preprocessor has no "
                    f"{preprocessor_method_name}()."
                ),
                exception=None,
            )
            return SingleFrameLocatorDiagnosticOutcome(
                result=None,
                error=error,
                trace_path=None,
                frame_hash=frame_hash,
        ...
        try:
            result = method(request, accepted_bytes)
        except Exception as exc:
            error = self._worker_error(
                request_id=request_id,
                frame_hash=frame_hash,
                frame=frame,
                failure_stage=FrameFailureStage.PREPROCESS,
                error_type="locator_diagnostic_failed",
                message=f"ROI locator diagnostic failed for request {request_id}: {exc}",
                exception=exc,
            )
            failure_trace_path = self._record_failure_trace(
                trace_dir=trace_dir,
        ...
        if record_trace and trace_dir is not None and self._trace_recorder is not None:
            try:
                trace_dir = self._trace_recorder.record_trace(
                    ...
                    image_bytes=accepted_bytes,
                    request=request,
                    prepared_inputs=_prepared_inputs_from_locator_diagnostic(
                        request,
                        result,
                    ),
                    result=None,
                ...
                trace_error = self._worker_error(
                    ...
                    failure_stage=FrameFailureStage.OUTPUT,
                    error_type="trace_recording_failed",
                    message=f"ROI locator diagnostic completed, but trace recording failed: {exc}",
                    exception=exc,
                )
                trace_dir = None
...
def _prepared_inputs_from_locator_diagnostic(
    request: InferenceRequest,
    result: object,
) -> PreparedInferenceInputs:
    metadata = getattr(result, "preprocessing_metadata", {})
    return PreparedInferenceInputs(
        request_id=request.request_id,
        input_keys=(),
        model_inputs={},
        source_frame=request.frame,
        preprocessing_metadata=metadata if isinstance(metadata, Mapping) else {},
        extras={"locator_diagnostic": True},
    )
