"""Evaluation entrypoints for saved 2D CNN runs."""
...
from .model_2d_cnn import DistanceRegressor2DCNN
...
def _load_model_from_run(
    ...
    architecture_variant = str(run_config.get("model_architecture_variant", "plain_v0_1"))
...
def evaluate_saved_run(config: EvalConfig | dict[str, Any]) -> dict[str, Any]:
    ...
    model = _load_model_from_run(run_dir, run_config=run_config, device=device)
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate a saved 2D CNN run.")
