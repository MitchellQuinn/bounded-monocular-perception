    ...
    *,
    ...
    upsert_preprocessing_contract_v2(
        ...
        current_representation={
            "Kind": f"silhouette_{mode}_array",
            "RepresentationMode": mode,
            "StorageFormat": "npy",
    ...
    optional_values_buffer: dict[str, list[str]] = {
        column: []
        for column in ["silhouette_image_filename", "silhouette_edge_debug_filename", "npy_filename"]
    ...
    for row_idx in samples_df.index:
        try:
            ...
            silhouette_filename = samples_df.at[row_idx, "silhouette_image_filename"]

            try:
                output_rel = normalize_relative_filename(silhouette_filename, new_suffix=".npy")
            except Exception as exc:
                samples_df.at[row_idx, "npy_filename"] = ""
                samples_df.at[row_idx, "npy_stage_status"] = "failed"
                samples_df.at[row_idx, "npy_stage_error"] = f"Invalid silhouette filename: {exc}"
                samples_df.at[row_idx, "pack_stage_status"] = "failed"
                samples_df.at[row_idx, "pack_stage_error"] = f"Invalid silhouette filename: {exc}"
                logger.log(f"Row {row_idx} failed: invalid silhouette filename {silhouette_filename}")
            ...
            silhouette_path = resolve_manifest_path(source_paths.root, "images", silhouette_filename)
            ...
            try:
                silhouette_gray = read_grayscale_uint8(silhouette_path)
