...
def _roi_metadata_from_preprocessing(metadata: Mapping[str, Any]) -> RoiMetadata | None:
    ...
    return RoiMetadata(
        locator_kind=_locator_kind(metadata),
        locator_result_accepted=_optional_boolish(
            metadata.get(contracts.PREPROCESSING_METADATA_LOCATOR_RESULT_ACCEPTED)
        ),
        locator_confidence=_optional_float(
            metadata,
        ...
        roi_requested_xyxy_px=_optional_float_tuple(
            metadata,
            contracts.PREPROCESSING_METADATA_ROI_REQUESTED_XYXY_PX,
            contracts.PREPROCESSING_METADATA_ROI_REQUEST_XYXY_PX,
            width=4,
        ),
        roi_source_xyxy_px=_optional_float_tuple(
            metadata,
            contracts.PREPROCESSING_METADATA_ROI_SOURCE_XYXY_PX,
            width=4,
        ),
        roi_clipped=_optional_boolish(
            metadata.get(contracts.PREPROCESSING_METADATA_ROI_CLIPPED)
        ),
        roi_clip_amount_px=_roi_clip_amount(metadata),
        roi_rejection_reasons=_string_tuple(
            metadata.get(contracts.PREPROCESSING_METADATA_ROI_REJECTION_REASONS)
        ),
...
def _locator_kind(metadata: Mapping[str, Any]) -> contracts.LocatorKind | None:
    raw = metadata.get(contracts.PREPROCESSING_METADATA_LOCATOR_KIND)
    if raw is None:
        return None
    try:
        return contracts.LocatorKind(str(raw))
    except ValueError:
        return None
...
def _roi_clip_amount(metadata: Mapping[str, Any]) -> dict[str, int]:
    mapping = {
        "left": contracts.PREPROCESSING_METADATA_ROI_CLIP_LEFT_PX,
        "right": contracts.PREPROCESSING_METADATA_ROI_CLIP_RIGHT_PX,
        "top": contracts.PREPROCESSING_METADATA_ROI_CLIP_TOP_PX,
        "bottom": contracts.PREPROCESSING_METADATA_ROI_CLIP_BOTTOM_PX,
    }
    values: dict[str, int] = {}
    for name, key in mapping.items():
        if key not in metadata or metadata[key] is None:
            continue
        values[name] = int(metadata[key])
    return values
