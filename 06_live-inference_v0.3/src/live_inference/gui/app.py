"""Runnable PySide6 application for live inference v0.3."""
...
LIVE_INFERENCE_APP_VERSION = "v0.3"


@dataclass(frozen=True)
class LiveInferenceGuiContext:
    """Composed worker controllers and shared state for the GUI."""
...
class _RuntimeDependencies:
    ...
    stage_transform_policy_state_cls: type
    ...
    roi_fcn_legacy_locator_adapter_cls: type
...
def build_live_inference_gui_context(
    *,
    ...
    locator_kind: str | contracts.LocatorKind = contracts.LocatorKind.BACKGROUND_EDGE_V1,
    ...
    **_legacy_ignored: object,
    ...
    kind = _locator_kind(locator_kind)
    device_override = (
    ...
    roi_fcn_device = (
        normalize_torch_device_policy(device_override or selection.roi_fcn_device)
        if getattr(selection, "roi_fcn_root", None) is not None
        else None
    )
    manifest = deps.load_live_model_manifest(
        selection.distance_orientation_root,
        roi_locator_root=selection.roi_fcn_root
        if kind == contracts.LocatorKind.ROI_FCN_LEGACY
        else None,
    )
    ...
    stage_policy_state = deps.stage_transform_policy_state_cls()
    ...
    locator_parameter_state = deps.locator_runtime_parameter_state_cls(
        deps.background_edge_locator_config_cls(
            ...
            roi_height_px=int(roi_wh[1]),
            background_threshold=int(background_threshold),
            min_foreground_area_px=int(min_foreground_area_px),
            ...
            dilation_kernel_px=int(dilation_kernel_px),
    ...
    locator = _build_locator(
        ...
        roi_wh=roi_wh,
        roi_fcn_device=roi_fcn_device or "auto",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        context = build_live_inference_gui_context(
            ...
            min_foreground_area_px=args.min_foreground_area,
            canny_low_threshold=args.canny_low,
            canny_high_threshold=args.canny_high,
            morphology_close_kernel_px=args.morphology_close_kernel,
            dilation_kernel_px=args.dilation_kernel,
        ...
        if args.auto_start_inference:
            QTimer.singleShot(0, window.start_continuous_inference)
        try:
            return int(app.exec())
        finally:
...
def _argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--frame-interval-ms", type=_positive_int, default=DEFAULT_FRAME_INTERVAL_MS)
    parser.add_argument("--camera-device", default=DEFAULT_CAMERA_DEVICE)
    parser.add_argument("--camera-width", type=_positive_int, default=DEFAULT_CAMERA_WIDTH_PX)
    parser.add_argument("--camera-height", type=_positive_int, default=DEFAULT_CAMERA_HEIGHT_PX)
    parser.add_argument("--camera-fps", type=_positive_int, default=DEFAULT_CAMERA_FPS)
    parser.add_argument("--camera-pixel-format", default=DEFAULT_CAMERA_PIXEL_FORMAT)
    parser.add_argument("--camera-encoding", default=DEFAULT_CAMERA_ENCODING)
    parser.add_argument("--camera-name", default=DEFAULT_CAMERA_NAME, help=argparse.SUPPRESS)
    parser.add_argument("--log-v4l2-controls-at-startup", action="store_true", help=argparse.SUPPRESS)
    ...
    parser.add_argument("--device", type=_device_policy, default=None, metavar="{auto,cuda,cpu}")
    parser.add_argument(
        "--locator-kind",
        choices=tuple(kind.value for kind in contracts.LocatorKind),
        default=contracts.LocatorKind.BACKGROUND_EDGE_V1.value,
    )
    parser.add_argument("--background-threshold", type=_non_negative_int, default=25)
    parser.add_argument("--min-foreground-area", type=_non_negative_int, default=250)
    parser.add_argument("--canny-low", type=_non_negative_int, default=40)
    parser.add_argument("--canny-high", type=_non_negative_int, default=120)
    parser.add_argument("--morphology-close-kernel", type=_non_negative_int, default=5)
    parser.add_argument("--dilation-kernel", type=_non_negative_int, default=3)
    parser.add_argument("--roi-clip-tolerance-px", type=_non_negative_int, default=0)
    parser.add_argument("--min-candidate-score", type=float, default=0.05)
    parser.add_argument("--inference-poll-interval-ms", type=_non_negative_int, default=DEFAULT_INFERENCE_POLL_INTERVAL_MS, help=argparse.SUPPRESS)
    parser.add_argument("--selection-path", dest="model_selection", type=Path, help=argparse.SUPPRESS)
    ...
    parser.add_argument("--roi-locator-polarity", default=None, help=argparse.SUPPRESS)
    parser.add_argument("--roi-locator-input-mode", default=None, help=argparse.SUPPRESS)
    parser.add_argument("--diagnostic-profile", default=None, help=argparse.SUPPRESS)
    return parser
...
def _load_runtime_dependencies() -> _RuntimeDependencies:
    ...
    return _RuntimeDependencies(
        ...
        stage_transform_policy_state_cls=StageTransformPolicyState,
        ...
        roi_fcn_legacy_locator_adapter_cls=RoiFcnLegacyLocatorAdapter,
...
def _build_locator(
    ...
    *,
    ...
    if kind == contracts.LocatorKind.BACKGROUND_EDGE_V1:
        return deps.background_edge_locator_cls(
            background_state=background_state,
            parameter_state=locator_parameter_state,
        )
    if kind == contracts.LocatorKind.FIXED_CENTER_ROI:
        return deps.fixed_center_roi_locator_cls(roi_wh_px=roi_wh)
    if kind == contracts.LocatorKind.MANUAL_FIXED_ROI:
        return deps.manual_fixed_roi_locator_cls(roi_wh_px=roi_wh)
    if kind == contracts.LocatorKind.ROI_FCN_LEGACY:
        roi_root = getattr(selection, "roi_fcn_root", None)
        if roi_root is None:
            raise ValueError("locator-kind=roi_fcn_legacy requires a [roi_fcn] selection.")
        legacy = deps.roi_fcn_locator_cls(roi_root, device=roi_fcn_device)
        return deps.roi_fcn_legacy_locator_adapter_cls(legacy)
    raise ValueError(f"Unsupported locator kind: {kind.value}")
...
def _locator_kind(value: str | contracts.LocatorKind) -> contracts.LocatorKind:
    if isinstance(value, contracts.LocatorKind):
        return value
    return contracts.LocatorKind(str(value))
...
def _print_launch_debug(context: LiveInferenceGuiContext) -> None:
    ...
    print(f"  app_version = {LIVE_INFERENCE_APP_VERSION}", file=sys.stderr)
    ...
    print(f"  locator_kind = {context.locator_kind.value}", file=sys.stderr)
    print(
